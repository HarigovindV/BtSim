...
void Battery::RunSim(void)
{
	...
	{
	//GetindividualCellVoltages
	struct CellHolder CellSorter[3]={{CellOne,1,0},{CellTwo,2,0},{CellOne,3,0}};
...
void Battery::SetSwitchState(Simulator::CellHolder * In,Simulator::SwitchHolder * SwIn)
{
	...
	{
		SwIn[In[0].CellCount].Switch.SetSwitch(1);
		SwIn[In[1].CellCount].Switch.SetSwitch(1);
		SwIn[In[2].CellCount].Switch.SetSwitch(1);
	}
	else if(In[1].Cell.GetPotential()-In[0].Cell.GetPotential()<50)
	{
		SwIn[In[0].CellCount].Switch.SetSwitch(0);
		SwIn[In[1].CellCount].Switch.SetSwitch(1);
		SwIn[In[2].CellCount].Switch.SetSwitch(1);
	}
	else
	{
		SwIn[In[0].CellCount].Switch.SetSwitch(0);
		SwIn[In[1].CellCount].Switch.SetSwitch(0);
		SwIn[In[2].CellCount].Switch.SetSwitch(1);	
	}
...
void Battery::DeduceCurrents(Simulator::CellHolder * In,float &CurrentOne,float &CurrentTwo,float &CurrentThree)
{
	...
	{
		TotalResistanceCellComponent+=1/(ResistorTwo.GetResistance()+CellTwo.GetInternalResistance().GetResistance());
	}
